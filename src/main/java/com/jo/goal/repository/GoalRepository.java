...
//    List<Goal> findTop3ByStateAndUserIdOrderByIdDesc(int state, Long userId); // state 값에 따라 내림차순으로 3개만 조회하는 쿼리
    List<Goal> findByStateAndUserIdOrderByIdDesc(int state, Long userId); // state 값에 따라 내림차순으로 목표 리스트 조회
    ...
    List<Goal> findByStateAndUserIdAndGoalTitleContaining(int state, Long userId, String keyword); // 목표 title로 검색
}